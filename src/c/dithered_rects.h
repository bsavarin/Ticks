...
void set1_pixel(uint8_t *bitmap_data, int bytes_per_row, int y, int x, uint8_t color) {
   bitmap_data[y*bytes_per_row + x] = color; // in Basalt - simple set entire byte
}

//Get pixel color at given coordinates 
uint8_t get1_pixel(uint8_t *bitmap_data, int bytes_per_row, int y, int x) {
...
void draw_zero_percent_mask(uint8_t *bitmap_data, int bytes_per_row, int x_start, int y_start, int width, int height, GColor first, GColor mask){
  GColor temp_pixel;
	...
	for(int x = 0; x < width; x++){
      ...
      temp_pixel = (GColor)get1_pixel(bitmap_data, bytes_per_row, y + y_start, x + x_start);
      
      if (gcolor_equal(temp_pixel, mask)) {
        set1_pixel(bitmap_data, bytes_per_row, y + y_start, x + x_start, first.argb);
      }
  ...
  GColor temp_pixel;
	...
	for(int x = 0; x < width; x++){
      ...
      if (gcolor_equal(temp_pixel, mask)) {
        ...
        set1_pixel(bitmap_data, bytes_per_row, y + y_start, x + x_start, temp_pixel.argb);
      }
